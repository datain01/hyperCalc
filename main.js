...
var hyperPointElement = document.getElementById("hyperPointBase");
var basePoint;
inputBox.addEventListener("input", function () {
    var input = inputBox.value;
    var n = parseInt(input, 10); // Parse input as an integer
    var lastDigit = n % 10;
    if (n >= 140 && n <= 149) {
    ...
    if (hyperPointElement) {
        hyperPointElement.textContent = "\uACB0\uACFC: ".concat(basePoint);
    }
});
// 숫자 입력란 id 배열
var inputIds = [];
for (var i = 1; i <= 17; i++) {
    inputIds.push("numberInput".concat(i));
}
// 결과 값 배열
var results = [];
// 각 입력란에 대한 처리
var statIncreaseElements = [];
inputIds.forEach(function (inputId, index) {
    ...
    inputElement.addEventListener("input", function () {
        ...
        var usedPointSum = usedPoint.reduce(function (total, point) { return total + point; }, 0);
        var remainingPoint = basePoint - usedPointSum;
    ...
    var statIncreaseElement = document.getElementById("statIncrease".concat(index + 1));
